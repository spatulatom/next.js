...
Next.js wraps the [`fetch` API](#fetch) requests using the `GET` or `HEAD` methods (with custom logic and React `cache` function) to automatically **memoize** those requests that have the same URL and options. This means you can call a fetch function for the same data in multiple places in a React component tree while only executing it once.
